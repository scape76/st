...
TEST_F(SubjectBuilderTest, RequiredFieldValidation) {
  ...
  // Missing code
  EXPECT_THROW(builder.setName("Test Subject").build(), std::invalid_argument);
}
...
TEST_F(TaskBuilderTest, RequiredFieldValidation) {
  ...
  EXPECT_THROW(builder.setTitle("Test Task").setDeadline(testDeadline).build(),
               std::invalid_argument);
}
...
TEST_F(TaskBuilderTest, ResetBuilder) {
  ...
  auto task2 =
      builder.setTitle("Test Exam").setDeadline(testDeadline).asExam().build();
...
TEST_F(TaskBuilderTest, HandlesInvalidDeadline) {
  ...
  auto pastDeadline = std::chrono::system_clock::now() - std::chrono::hours(24);

  // This should either throw an exception or handle it gracefully depending on
