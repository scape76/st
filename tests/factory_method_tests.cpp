...
TEST_F(TaskFactoryTest, LabFactoryCreatesLabTask) {
  auto task = labFactory->createTask(testTitle, testDeadline, testDescription);
...
TEST_F(TaskFactoryTest, ExamFactoryCreatesExamTask) {
  auto task = examFactory->createTask(testTitle, testDeadline, testDescription);
...
TEST_F(TaskFactoryTest, CreateTaskWithDefaultDescription) {
  ...
  EXPECT_EQ(testTitle, task->getTitle());
  EXPECT_EQ("", task->getDescription()); // Default description should be empty
  EXPECT_EQ(testDeadline, task->getDeadline());
  EXPECT_EQ("Lab", task->getType());

  // Verify with other factories too
  auto projectTask = projectFactory->createTask("Project Test", testDeadline);
  EXPECT_EQ("", projectTask->getDescription());
